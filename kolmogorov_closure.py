...
SPIN = int(os.environ.get('SPIN', 30000)); SNAP = int(os.environ.get('SNAP', 1200)); STEPS = int(os.environ.get('STEPS', 3000)); ROLL = int(os.environ.get('ROLL', 2000))
...
class CNN(nn.Module):
    ...
    def forward(self, w): return self.net(w[:, None])[:, 0]
...
def train(model, name):
    ...
    for step in range(STEPS):
        i = torch.randint(0, ntr - 5, (16,), generator=g); w = Wtr[i]; loss = 0
        for h in range(4):                    # 4-step rollout loss
            w = model.step(w); loss = loss + F.mse_loss(w, Wtr[i + h + 1])
        opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0); opt.step()
        if step % 500 == 499: print('  %-12s step %4d  loss %.5f  %.0fs' % (name, step + 1, loss.item() / 4, time.time() - t0), flush=True)
    return model.eval()
...
for name, m in arms.items():
    ...
    for s0 in (0, n_avail // 3, 2 * n_avail // 3):
        ...
        spec_err.append((torch.log(spectrum(R[-1]) + 1e-12) - torch.log(spectrum(T[-1]) + 1e-12))[1:].abs().mean().item())
    results[name] = (np.mean(hor), min(edrift), max(edrift), np.mean(spec_err))
    print('%-12s  horizon(corr>0.9) %6.0f steps | energy/truth min %.2f max %.2f | log-spectrum error %.2f' % (name, *results[name]), flush=True)
a, b = results['learned'], results['frozen+gate']
print('\nREGISTERED  horizon ratio frozen+gate/learned = %.1f (>=3)  -> %s' % (b[0] / max(a[0], 1), 'PASS' if b[0] >= 3 * a[0] else 'FAIL'))
print('            frozen+gate energy band [%.2f, %.2f] within [0.5,1.5] -> %s | learned band [%.2f, %.2f] leaves it -> %s' % (b[1], b[2], 'PASS' if 0.5 <= b[1] and b[2] <= 1.5 else 'FAIL', a[1], a[2], 'yes' if a[1] < 0.5 or a[2] > 1.5 else 'no'))
