...
print("   t     delta (n=%g fixed)   delta_free   n_free   E/E0" % NFIX)
...
while t < T + 1e-9:
    if t >= mark - 1e-9:
        ...
        if d > dx2:
            ts.append(t)
            ds.append(d)
        print("%5.2f   %.4f              %.4f      %+.2f    %.8f%s" % (t, d, df, n, E / E0, "" if d > dx2 else "   <-- below 2dx"), flush=True)
        mark += EVERY
...
sel = ts > 0.15    # skip the transient where the k^-n fit is meaningless (initial data has a single shell)
ts, ds = ts[sel], ds[sel]
