...
def k_grid(N, dims):
    ...
    return np.meshgrid(*([k] * dims), indexing="ij")
...
def tg2d(N=128, nu=0.02, T=2.0, dt=2e-3):
    ...
    while t < T - 1e-12:
        ...
        b = nl(fac2 * (wh + dt / 2 * a))
        c = nl(fac2 * wh + dt / 2 * b)
        d = nl(fac * wh + dt * fac2 * c)
        wh = fac * wh + dt / 6 * (fac * a + 2 * fac2 * b + 2 * fac2 * c + d)
        t += dt
        if abs(t - round(t * 2) / 2) < 1e-9:
            u, v = vel(wh)
            ut = np.exp(-2 * nu * t) * np.sin(X) * np.cos(Y)
            vt = -np.exp(-2 * nu * t) * np.cos(X) * np.sin(Y)
...
if os.environ.get("ONLY3D") != "1":
    ...
    for t, e, ex, err in tg2d():
        print("%4.1f   %.8f     %.8f     %.2e" % (t, e, ex, err))
...
def tg3d(N, T=4.0, dt=None):
    ...
    def enstrophy(U):
        u = U
        wx = 1j * ky * u[2] - 1j * kz * u[1]
        wy = 1j * kz * u[0] - 1j * kx * u[2]
        wz = 1j * kx * u[1] - 1j * ky * u[0]
    ...
    while t < T - 1e-12:
        ...
        if abs(t - round(t)) < dt / 2:
            rows.append((round(t), energy(U) / E0, enstrophy(U)))
