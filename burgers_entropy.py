...
def upwind_step(u, dt, nu):
    ...
    fp = np.roll(f, -1)
    a = u
    flux_l = np.where(a > 0, fm, f)
    flux_r = np.where(a > 0, f, fp)  # upwind choice
...
def frozen_step(uh, dt, nu):
    ...
    k1 = rhs_spectral(uh, nu)
    k2 = rhs_spectral(uh + 0.5 * dt * k1, nu)
    k3 = rhs_spectral(uh + 0.5 * dt * k2, nu)
    k4 = rhs_spectral(uh + dt * k3, nu)
    return uh + dt / 6 * (k1 + 2 * k2 + 2 * k3 + k4)


def grad_max(u):
    return np.abs(np.fft.ifft(1j * k * np.fft.fft(u)).real).max()
...
for nu in (0.0, 0.01):
    print(
        ...
        % (
            nu,
            (
                "inviscid: gradient blows up at t=1, energy exactly conserved"
                if nu == 0
                else "viscous: dissipation should equal nu*mean(u_x^2), nothing more"
            ),
        )
    )
    u_up = u0.copy()
    ...
    print(
        "%6s | %-38s | %-38s | %s"
        % ("t", "upwind: E/E0, max|u_x|, L2err", "frozen: E/E0, max|u_x|, L2err", "truth max|u_x|")
    )
    Eup_prev, Efr_prev, tprev = E0, E0, 0.0
    while mi < len(marks):
        u_up = upwind_step(u_up, DT, nu)
        uh = frozen_step(uh, DT, nu)
        t += DT
        if t >= marks[mi] - 1e-9:
            u_fr = np.fft.ifft(uh).real
            tr = truth(t) if nu == 0 else None
            e_up, e_fr = energy(u_up), energy(u_fr)
            if nu == 0:
                print(
                    "%6.2f | %.5f  %7.2f  %.4f            | %.5f  %7.2f  %.4f            | %7.2f"
                    % (
                        t,
                        e_up / E0,
                        grad_max(u_up),
                        np.sqrt(np.mean((u_up - tr) ** 2)),
                        e_fr / E0,
                        grad_max(u_fr),
                        np.sqrt(np.mean((u_fr - tr) ** 2)),
                        1 / (1 - t),
                    )
                )
            else:
                phys_up = nu * np.mean(np.gradient(u_up, dx) ** 2)
                phys_fr = nu * np.mean(np.fft.ifft(1j * k * uh).real ** 2)
                meas_up = -(e_up - Eup_prev) / (t - tprev) / 2
                meas_fr = -(e_fr - Efr_prev) / (t - tprev) / 2
                print(
                    "%6.2f | measured dissipation %.4f vs physical %.4f (x%.2f) | measured %.4f vs physical %.4f (x%.2f)"
                    % (t, meas_up, phys_up, meas_up / phys_up, meas_fr, phys_fr, meas_fr / phys_fr)
                )
            Eup_prev, Efr_prev, tprev = e_up, e_fr, t
            mi += 1
    if nu == 0:
        print(
            "numerical entropy production over [0,0.95]:  upwind %.3e /unit time   frozen %.3e /unit time"
            % (-np.log(energy(u_up) / E0) / 0.95, -np.log(energy(np.fft.ifft(uh).real) / E0) / 0.95)
        )
