...
# ------------------------------------------------ 1-D Burgers ------------------------------------------------
N = 512; x = np.linspace(0, 2 * np.pi, N, endpoint=False); dx = x[1] - x[0]; k = np.fft.fftfreq(N, d=1.0 / N); deal = np.abs(k) < N / 3
def D(u, n=1): return np.fft.ifft((1j * k) ** n * np.fft.fft(u)).real
def rhs1(uh, nu):
...
def frozen1(uh, dt, nu):
    a = rhs1(uh, nu); b = rhs1(uh + dt / 2 * a, nu); c = rhs1(uh + dt / 2 * b, nu); d = rhs1(uh + dt * c, nu)
    return uh + dt / 6 * (a + 2 * b + 2 * c + d)
def upwind1(u, dt, nu):
    f = 0.5 * u * u; fm = np.roll(f, 1); fp = np.roll(f, -1); fl = np.where(u > 0, fm, f); fr = np.where(u > 0, f, fp)
...
def quantities1(u):
    ux = D(u, 1); return dict(energy=0.5 * np.mean(u ** 2), gradient=0.5 * np.mean(ux ** 2))

def run1(nu, T, dt=1e-4, checks=(0.3, 0.6, 0.8)):
    u0 = np.sin(x); uh = np.fft.fft(u0); uu = u0.copy(); t = 0.0; rows = []; maxu0 = np.abs(u0).max()
    ci = 0
    while ci < len(checks):
        # measure d/dt by centred difference across one step, evaluate rhs at the midpoint field
        uh_prev = uh.copy(); uu_prev = uu.copy()
        uh = frozen1(uh, dt, nu); uu = upwind1(uu, dt, nu); t += dt
        if t >= checks[ci] - 1e-12:
            for name, before, after, mid in (('frozen', np.fft.ifft(uh_prev).real, np.fft.ifft(uh).real, np.fft.ifft(frozen1(uh_prev, dt / 2, nu)).real),
                ...
                for key in ('energy', 'gradient'):
                    meas = (qa[key] - qb[key]) / dt; rhs = r[key]; scale = max(abs(meas), abs(rhs), 1e-300)
...
for nu, T in ((0.0, 0.8), (0.02, 0.8)):
    print('\n==== 1-D Burgers, nu = %.2f  (%s) ====' % (nu, 'inviscid: H false, gradient production drives blow-up' if nu == 0 else 'viscous: H true via the maximum principle'))
    print('  t    scheme   budget     measured d/dt    right-hand side   relative residual')
    for t, name, key, meas, rhs, res in run1(nu, T):
        if rhs is None: print('%4.2f   %-7s  %-10s %.6f' % (t, name, key, meas))
        else: print('%4.2f   %-7s  %-10s %+.6e   %+.6e   %.1e' % (t, name, key, meas, rhs, res))

# --------------------------------------------- 2-D decaying turbulence ---------------------------------------------
M = 128; km = np.fft.fftfreq(M, d=1.0 / M); kx, ky = np.meshgrid(km, km, indexing='ij'); k2 = kx * kx + ky * ky; k2s = k2.copy(); k2s[0, 0] = 1.0
deal2 = (np.abs(kx) < M / 3) & (np.abs(ky) < M / 3); NU2 = 1e-3
def vel2(wh): psih = wh / k2s; psih[0, 0] = 0; return np.fft.ifft2(1j * ky * psih).real, -np.fft.ifft2(1j * kx * psih).real
def rhs2(wh):
    u, v = vel2(wh); w = np.fft.ifft2(wh).real; wx = np.fft.ifft2(1j * kx * wh).real; wy = np.fft.ifft2(1j * ky * wh).real
    adv = np.fft.fft2(u * wx + v * wy); div = 1j * kx * np.fft.fft2(u * w) + 1j * ky * np.fft.fft2(v * w)
    return -0.5 * (adv + div) * deal2 - NU2 * k2 * wh
def step2(wh, dt):
    a = rhs2(wh); b = rhs2(wh + dt / 2 * a); c = rhs2(wh + dt / 2 * b); d = rhs2(wh + dt * c); return wh + dt / 6 * (a + 2 * b + 2 * c + d)
def q2(wh):
    u, v = vel2(wh); w = np.fft.ifft2(wh).real; wx = np.fft.ifft2(1j * kx * wh).real; wy = np.fft.ifft2(1j * ky * wh).real
    return dict(energy=0.5 * np.mean(u * u + v * v), enstrophy=0.5 * np.mean(w * w), palinstrophy=0.5 * np.mean(wx * wx + wy * wy))
def b2(wh):
    u, v = vel2(wh); w = np.fft.ifft2(wh).real; wx = np.fft.ifft2(1j * kx * wh).real; wy = np.fft.ifft2(1j * ky * wh).real
    ux = np.fft.ifft2(1j * kx * np.fft.fft2(u)).real; uy = np.fft.ifft2(1j * ky * np.fft.fft2(u)).real
    vx = np.fft.ifft2(1j * kx * np.fft.fft2(v)).real; vy = np.fft.ifft2(1j * ky * np.fft.fft2(v)).real
    lapw = np.fft.ifft2(-k2 * wh).real
    prod = -np.mean(wx * (ux * wx + uy * wy) + wy * (vx * wx + vy * wy))       # -<grad w . (grad u) grad w>, no sign
    return dict(energy=-NU2 * np.mean(w * w), enstrophy=-NU2 * np.mean(wx * wx + wy * wy), palinstrophy=prod - NU2 * np.mean(lapw ** 2)), prod
...
for tc in (0.5, 1.0, 2.0, 4.0):
    while t < tc - 1e-12:
        whp = wh.copy(); wh = step2(wh, dt); t += dt
    mid = step2(whp, dt / 2); qb, qa = q2(whp), q2(wh); r, prod = b2(mid)
    for key in ('energy', 'enstrophy', 'palinstrophy'):
        meas = (qa[key] - qb[key]) / dt; rhs = r[key]; scale = max(abs(meas), abs(rhs), 1e-300)
        print('%4.1f   %-13s %+.6e   %+.6e   %.1e%s' % (t, key, meas, rhs, abs(meas - rhs) / scale, ('   production %+.4e' % prod) if key == 'palinstrophy' else ''))
